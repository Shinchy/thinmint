...
ThinMint.RpcQueue.prototype.verifyResponse = function(response) {
  ...
  if(jQuery.isArray(response) && that.getMethodsLength() != response.length) {
    console.error('ThinMint.RpcQueue.run', 'Response mismatch.  Expected ' + that.getMethodsLength() + ' and received ' + response.length);
...
ThinMint.RpcQueue.prototype.fetch = function(callback) {
  ...
  console.info('ThinMint.RpcQueue.run', 'Requesting data for:', this.getMethods());
  ...
  jQuery.ajax({
    ...
    success: function(response) {
      ...
      console.debug('ThinMint.RpcQueue.run', 'Finished in: ' + (+(new Date())-benchmarkStart) + 'ms', that.getMethods());
    },
    error: function() {
